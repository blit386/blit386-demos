...
Claude Code's `.claude/settings.json` runs `rtk hook claude` on Bash. Prefer shell + RTK (`rtk read`, `rtk grep`, `git`,
`pnpm run …`) over native Read/Grep for exploration. Full policy: `~/.claude/RTK.md`.
